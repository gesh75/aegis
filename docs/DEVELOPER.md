...
| GET | `/api/status` | Public. Field contract below — no secrets. |
...
`GET /api/status` fields (`serve.status`, public, no auth):

| Field | Values | Source |
|---|---|---|
| `api_auth` | `required` / `open` | `AEGIS_API_KEY` set / unset |
| `approve_hmac` | `required` / `asserted-unverified` | `AEGIS_APPROVE_KEY` usable / unset |
| `seal` | `pinned` / `ephemeral` | `AEGIS_SEAL_KEY` set / unset |
| `egress` | always `"none"` | hard-pinned |

Pairing-hole detector: `api_auth=open` **and** `approve_hmac=required` means mint
is reachable with no `X-Aegis-Key`. That is current `main` — draft #28 is not
shipped. A process that **starts** with `AEGIS_SEAL_KEY` set and `AEGIS_API_KEY`
unset is `SystemExit` (HMAC mint is not gated the same way).
...
Confirm pairing with the public status route (no header needed):

```bash
curl -s http://127.0.0.1:8088/api/status
# safe:     {"api_auth":"required","approve_hmac":"required","seal":"…","egress":"none"}
# hole:     {"api_auth":"open","approve_hmac":"required",…}   # mint is unauthenticated
# honesty:  {"api_auth":"…","approve_hmac":"asserted-unverified",…}  # mint 503
```
...
Debug a v2 deny by recomputing the same digests the gate uses
(`tokens.config_digest` / `tokens.inventory_digest`):

```python
from aegis.core.promote.tokens import config_digest, inventory_digest, digests_from_bundle
digest, cfg, inv = digests_from_bundle(bundle)
# cfg  == SHA-256 of sorted {device, vendor, config} rows
# inv  == SHA-256 of {devices, engine, inventory_rev|rev, node_count, topology}
# A live promote override replaces inv before G2/G3 — compare that hex, not cfg.
```
